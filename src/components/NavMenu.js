...
class NavMenu extends PureComponent {
    ...
    render() {
        ...
        let MenuList = this.menuLis.map((item, index) => item.children ? (
            <SubMenu key={"sub"+ index+1} icon={item.type} title={item.title}>
                {item.children.map((chd) => (<Menu.Item key={chd.path}>{chd.title}</Menu.Item>))}
            </SubMenu>
        ): (<Menu.Item key={item.path} icon={item.type}>{item.title}</Menu.Item>))
